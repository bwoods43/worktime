...
<?php // 9/5/10 baw add verification process ?>
		<form method="post" action="/admin/reports/timesheet" class="basic_form verify_no">
		<p>I confirm that the hours listed here are accurate: 
			<input type="hidden" name="verify_status" value="new" />
			<input type="hidden" name="work_date" value="<?php echo createFriendlyDate($end_date, 'm/d/Y'); ?>" />	
			<input type="hidden" name="report_type" value="<?php echo $_GET['report_type']; ?>" />			

			<input type="submit" class="button" value="Verify" />
		</p></form>
<?php if ($this->verify_info) { ?>
		<p class="verify_yes">This time has been verified by <?php echo $this->verify_info->first_name . " " . $this->verify_info->last_name . " at " . $this->verify_info->verify_date; ?>.</p>
<?php } ?>
